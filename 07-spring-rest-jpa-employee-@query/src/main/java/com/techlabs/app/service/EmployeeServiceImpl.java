...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public ResponseDto getEmployeeById(int id) {
		...
		return employeeObjectToResponseDto(byId);

	}

	@Override
	public ResponseDto updateEmployee(RequestDto requestDto) {

		Employee employee = requestDtoToEmployeeObject(requestDto);
		ResponseDto dto = employeeObjectToResponseDto(employeeRepository.update(employee.getId(), employee.getName(),
	...
	public List<ResponseDto> getAll() {
		return getList(employeeRepository.getAllByNative());
	}

	@Override
	public ResponseDto saveEmployee(@Valid RequestDto employee) {
		
		Employee employee1 = requestDtoToEmployeeObject(employee);
		employeeRepository.saveEmployee(employee1.getId(), employee1.getName(),
				employee1.getSalary(), employee1.getDesignation(),employee1.isActive());
		
		//if(i==0) throw new EmployeeNotFoundException( "Employee is not Added");
		return employeeObjectToResponseDto(employee1);
	}
